...
class ImportPendapatanCommand extends Command
{
    protected $signature = 'bms:import-pendapatan
        {--file=database/data/pendapatan_sawai_2026-06.csv : Path relatif ke CSV}
        {--branch=Sawai : Nama cabang tujuan}
        {--replace : Hapus dulu transaksi pemasukan cabang pada rentang tanggal CSV}
        {--dry-run : Hanya hitung, tidak insert}';

    protected $description = 'Impor transaksi pemasukan dari CSV pendapatan ke BMS';
    ...
    public function handle(): int
    {
        ...
        while (($data = fgetcsv($handle)) !== false) {
            ...
            if ($description === '') {
                ...
                $description = preg_replace('/^\s*Akun\s+CASH\s*\|\s*/i', '', $description) ?: null;
            }
        ...
        $this->info('Kategori income: '.$categories->keys()->map(fn ($k) => $categories[$k]->name)->unique()->implode(', '));
        $this->info("Rentang tanggal: {$minDate} s/d {$maxDate}");
        $this->info('Siap diimpor: '.count($rows)." baris | dilewati: {$skipped} | duplikat di CSV: {$dupInFile}");
        ...
        DB::transaction(function () use ($rows, $branch, $minDate, $maxDate) {
            if ($this->option('replace') && $minDate && $maxDate) {
                $incomeIds = Category::query()->where('type', 'income')->pluck('id');
                $deleted = Transaction::query()
                    ->where('branch_id', $branch->id)
                    ->whereIn('category_id', $incomeIds)
                    ->whereBetween('transaction_date', [$minDate, $maxDate])
                    ->delete();
                $this->warn("Replace: menghapus {$deleted} transaksi pemasukan lama pada rentang tersebut.");
            }
        ...
        $this->info('Impor selesai: '.count($rows).' transaksi pemasukan (Akun CASH).');
