...
/**
 * Import pemasukan konter Lukulamo Juli 2026 dari spreadsheet.
 */
...
if (! $xlsx || ! is_file($xlsx)) {
    $fallback = 'c:\\Users\\62823\\Downloads\\Untitled spreadsheet (6).xlsx';
...
$mapLabel = static function (string $raw) use ($findCat): array {
    ...
    if (str_contains($upper, 'PULSA')) {
        return [$findCat('Penjualan Pulsa', 'Pulsa'), 'Pulsa', $label];
    }
    if (str_contains($upper, 'HP') || str_contains($upper, 'ACC') || str_contains($upper, 'PENJUALAN')) {
        return [$findCat('Penjualan HP', 'Penjualan'), 'Penjualan HP ACC', $label];
    }

    return [$findCat($label, 'Lain-lain', 'Penjualan'), $label, $label];
};

$marker = 'Import spreadsheet pemasukan Juli 2026 Lukulamo';
